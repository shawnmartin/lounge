...
class CouchbaseDocument extends Document {
  ...
  _saveRefField (path, thing, options, fn) {
    function saveRefDoc (couchDoc) {
      console.log('############### saveRefDoc ##############')
      console.dir(couchDoc, { depth: 10 })
      couchDoc.save(options, (err, savedDoc) => {
        console.log('############### couchDoc.save ##############')
        console.dir(savedDoc, { depth: 10 })
        if (err) {
  ...
  static _findById (id, options, memo, fn) {
    ...
    if (Array.isArray(id)) {
      ...
      driver.get(id, (err, getRes) => {
        console.log('############### _findById ##############')
        console.dir(getRes, { depth: 10 })
        if (err) {
          return fn(err)
        }
